...
bool tx_mining_auth(WiFiClient &client, const char *user, const char *pass)
{
    ...
    Serial.printf("[TRABAJADOR] ==> Trabajo Autorizado\n");
    Serial.print("  Enviando  : ");
    Serial.println(payload);
    client.print(payload);
...
{
    Serial.println("    Analizando Método [NOTIFICACIÓN DE MINERÍA]");
    if (!verifyPayload(&line))
        return false;
...
#ifdef DEBUG_MINING
    Serial.print("    job_id: ");
    Serial.println(mJob.job_id);
    Serial.print("    prevhash: ");
    Serial.println(mJob.prev_block_hash);
    Serial.print("    coinb1: ");
    Serial.println(mJob.coinb1);
    Serial.print("    coinb2: ");
    Serial.println(mJob.coinb2);
    Serial.print("    merkle_branch size: ");
    Serial.println(mJob.merkle_branch.size());
    Serial.print("    version: ");
    Serial.println(mJob.version);
    Serial.print("    nbits: ");
    Serial.println(mJob.nbits);
    Serial.print("    ntime: ");
    Serial.println(mJob.ntime);
    Serial.print("    clean_jobs: ");
    Serial.println(mJob.clean_jobs);
#endif
...
{
    Serial.println("    Analizando Método [ESTABLECER DIFICULTAD]");
    if (!verifyPayload(&line))
        return false;
    ...
    Serial.println((double)doc["params"][0], 12);
