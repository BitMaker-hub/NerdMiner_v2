...
background.drawString(mensajericono, 100, 140, GFXFF); 
background.setTextSize(2);
background.drawString(data.temp.c_str(), 16, 115, GFXFF);
background.setTextSize(1);
background.drawString("c", 62, 111, GFXFF);
background.setTextColor(TFT_LIGHTGREY);
background.drawString("d", 155, 103, GFXFF);
background.setTextColor(colors[colorIndex]);
background.drawString("KH/s", 275, 87, GFXFF);
render.setCursor(11, 128);
render.setFontSize(14);
render.rdrawString(data.timeMining.c_str(), 260, 104, 0xDEDB);
background.pushSprite(0, 0);
}
...
void tDisplay_m8axScreen4(unsigned long mElapsed)
{
  ...
  if (actualizarc % 30==0) {
    ...
    tft.print("CPU: "); tft.print(getCpuFrequencyMhz()); tft.print(" MHz,"); tft.print(ESP.getChipCores()); tft.print("C");
} 
  actualizarc++;
} 
...
void tDisplay_m8axScreen15(unsigned long mElapsed)
{
    ...
    data.completedShares.c_str(), data.totalKHashes.c_str(), data.currentHashRate.c_str(), data.temp.c_str());
       ...
       tft.setTextColor(TFT_WHITE);
       float opsseg=(hashRa*1000)*10377;
       tft.setCursor(4,75);
       ...
       tft.print("TFLOPS");
       tft.setTextColor(TFT_WHITE);
       float tflops=((hashRa*1000)*10377)/1000000000000;
       tft.setCursor(4,100);
       ...
       tft.print("GOPS");
       tft.setTextColor(TFT_WHITE);
       float gops=((hashRa*1000)*10377)/1000000;
       tft.setCursor(4,125);
       tft.print(gops);
       tft.setTextColor(colors[colorI]);
       ...
       tft.print("MOPS");
       tft.setTextColor(TFT_WHITE);
       float Mops=((hashRa*1000)*10377)/1000000;
       ...
       tft.print("TOPS");
       tft.setTextColor(TFT_WHITE);
       float Tops=((hashRa*1000)*10377)/1000000000;
...
void tDisplay_m8axScreen18(unsigned long mElapsed) {
  ...
  int numeritos[6];
    ...
    tft.setCursor(2,104); 
    tft.print("Numeros - ");tft.print(numeritos[0]); tft.print(" "); tft.print(numeritos[1]); tft.print(" "); tft.print(numeritos[2]); tft.print(" "); tft.print(numeritos[3]); tft.print(" "); tft.print(numeritos[4]); tft.print(" "); tft.print(numeritos[5]);
    tft.setCursor(2,114);
    tft.print("Numero Objetivo - "+String(destino));
    tft.setCursor(160,114);
    int millonario = atoi(data.valids.c_str());
    if (millonario == 0) {
      tft.print("NoRICO - ");tft.print(data.currentHashRate);tft.print(" KH/s. ");tft.print(data.temp);tft.print(" g");
    } else {
      tft.print("SiRICO - ");tft.print(data.currentHashRate);tft.print(" KH/s. ");tft.print(data.temp);tft.print(" g");
    }
...
void monedaYdado(unsigned long mElapsed) {
  ...
  tft.setTextSize(1);
  tft.print("DADO - ");tft.print(dado);
  tft.setCursor(108, 99);
  tft.print("MONEDA - ");tft.print(moneda==0 ? "CARA" : "CRUZ");
  tft.setCursor(108, 110);
  tft.print(horaStr);
  tft.setCursor(108, 121);
  tft.print("TEMP - ");tft.print(data.temp.c_str());tft.print(" G");
  tft.setTextSize(2);
  tft.setCursor(46, 4);
  ...
  tft.setTextSize(2);
  tft.setTextDatum(MC_DATUM); // Centra el texto
  tft.print(numeroAEscrito(parteEntera,false)+" Con ");tft.print(numeroAEscrito(parteDecimal,true)+" KH/s.");
