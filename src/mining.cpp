...
bool checkPoolConnection(void)
{
  ...
  Serial.println("M8AX - Cliente No Conectado, Intentando Conectar");

  // Resolve first time pool DNS and save IP
  if (serverIP == IPAddress(1, 1, 1, 1))
  {
    WiFi.hostByName(Settings.PoolAddress.c_str(), serverIP);
    Serial.printf("M8AX - DNS Resuelto Y IP Guardada (Primera Vez) Obtenida: %s\n", serverIP.toString());
  }

  // Try connecting pool IP
  if (!client.connect(serverIP, Settings.PoolPort))
  {
    Serial.println("M8AX - Imposible Conectar A : " + Settings.PoolAddress);
    WiFi.hostByName(Settings.PoolAddress.c_str(), serverIP);
    Serial.printf("M8AX - DNS Resuelto, Obtenido: %s\n", serverIP.toString());
    return false;
  }
...
bool checkPoolInactivity(unsigned int keepAliveTime, unsigned long inactivityTime)
{
  ...
  {
    mLastTXtoPool = time_now;
    Serial.println("  M8AX - Enviando  : Mantener Conexión Activa Y Sugerir Dificultad");
    // if (client.print("{}\n") == 0) {
...
void runStratumWorker(void *name)
{
  ...
  Serial.printf("M8AX - Encontrados %d Trabajadores Esclavos\n", i2c_slave_vector.size());
  if (!i2c_slave_vector.empty())
  {
    Serial.print("   M8AX - Trabajadores: ");
  ...
  while (true)
  {
    ...
    {
      // Restart connection
      Serial.println("  M8AX - Detectados Más De 2 Minutos Sin Datos Del Servidor Stratum. Cerrando El Socket Y Reabriendo...");
      client.stop();
      isMinerSuscribed = false;
      MiningJobStop(job_pool, s_submition_map);
      continue;
    ...
    while (client.connected() && client.available())
    {
      ...
      case MINING_NOTIFY:
        ...
        {
          Serial.println("M8AX - Error De Análisis, Es Necesario Reiniciar...");
          client.stop();
          isMinerSuscribed = false;
          MiningJobStop(job_pool, s_submition_map);
        }
      ...
      case STRATUM_PARSE_ERROR:
      {
        ...
        if (itt != s_submition_map.end())
        {
          Serial.printf("M8AX - Rechazo De Envío %d\n", id);
          s_submition_map.erase(itt);
        }
      }
      break;
      default:
        Serial.println("  M8AX - JSON Analizado: Desconocido");
        break;
      }
    }
    ...
    while (!job_result_list.empty())
    {
      ...
      if (res->difficulty > currentPoolDifficulty && job_pool == res->id && res->nonce != 0xFFFFFFFF)
      {
        ...
        Serial.print("   - Acción De Dificultad Actual : ");
        Serial.println(res->difficulty, 12);
        Serial.print("   - Dificultad Actual De La Pool: ");
        Serial.println(currentPoolDifficulty, 12);
        Serial.print("   - TRANSACCIÓN TX              : ");
...
void restoreStat()
{
  ...
  {
    Serial.printf("[MONITOR-MVIIIAX] La Partición NVS Está Llena O Tiene Una Versión Inválida, Borrando...\n");
    nvs_flash_init();
  }
...
void saveStat()
{
  if (!Settings.saveStats)
    return;
  Serial.printf("[MONITOR-M8AX] Grabando Estadísticas\n");
...
void resetStat()
{
  Serial.printf("[MONITOR-M8AX] Reseteando Estadísticas De NVS\n");
...
void runMonitor(void *name)
{
  ...
  Serial.println("[MONITOR-M8AX] Comenzando...");
  restoreStat();
  ...
  while (1)
  {
    ...
    if (mElapsed >= 1000)
    {
      ...
      if (elapsedKHs == 0)
      {
        Serial.printf(">>> [i] Minero: NuevoTrabajo>%s / EnEjecución>%s) - Cliente: Conectado>%s / Suscrito>%s / WiFiConectado>%s\n",
                      "true", //(1) ? "true" : "false",
