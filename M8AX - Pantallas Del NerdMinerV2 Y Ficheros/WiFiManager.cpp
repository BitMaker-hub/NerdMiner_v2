  ...
  if (p->getID())
  {
    for (size_t i = 0; i < strlen(p->getID()); i++)
    {
      if (!(isAlphaNumeric(p->getID()[i])) && !(p->getID()[i] == '_'))
      {
        ...
        DEBUG_WM(WM_DEBUG_ERROR, F("[ERROR] Los ID De Parámetros Solo Pueden Contener Caracteres Alfanuméricos..."));
#endif
        return false;
      }
    }
...
// constructors
WiFiManager::WiFiManager(Print &consolePort)
    : _debugPort(consolePort)
{
  WiFiManagerInit();
}
...
// handle failure mode Brownout detector etc.
    ...
    if (WiFi.status() == WL_CONNECTED)
    {
      connected = true;
#ifdef WM_DEBUG_LEVEL
      DEBUG_WM(F("Auto Conexión: ESP Ya Conectado "));
#endif
      setSTAConfig();
...
// connected
#ifdef WM_DEBUG_LEVEL
      DEBUG_WM(F("Auto Conexión: CORRECTA"));
      DEBUG_WM(WM_DEBUG_VERBOSE, F("Connected in"), (String)((millis() - _startconn)) + " ms");
      DEBUG_WM(F("Dirección IP Establecida:"), WiFi.localIP());
#endif
...
#ifdef WM_DEBUG_LEVEL
    DEBUG_WM(F("Auto Conexión: FALLIDA Por"), (String)((millis() - _startconn)) + " ms");
#endif
  }
  else
  {
...
bool WiFiManager::startAP()
{
  bool ret = true;
#ifdef WM_DEBUG_LEVEL
  DEBUG_WM(F("Habilitado Punto De Acceso Con SSID:"), _apName);
#endif
  ...
  DEBUG_WM(F("Dirección IP De Punto De Acceso:"), WiFi.softAPIP());
#endif
...
#ifdef WM_DEBUG_LEVEL
  DEBUG_WM(F("Entrando En Portal WeB..."));
#endif
...
#ifdef WM_DEBUG_LEVEL
  DEBUG_WM(WM_DEBUG_NOTIFY, F("Saliendo Del Portal De Configuración..."));
#endif
  return result;
}
  ...
  if (connect)
  {
    ...
    else
    {
      ...
      if (res || (!_connectonsave))
      {
        ...
        {
          DEBUG_WM(F("Conexión A Nuevo Punto De Acceso [CORRECTA]"));
          DEBUG_WM(F("Adquirida Dirección IP:"));
...
#ifdef WM_DEBUG_LEVEL
      DEBUG_WM(WM_DEBUG_ERROR, F("[ERROR] Conexión Fallida Al Nuevo Punto De Acceso..."));
#endif
    }
...
uint8_t WiFiManager::connectWifi(String ssid, String pass, bool connect)
{
  ...
  while (retry <= _connectRetries && (connRes != WL_CONNECTED))
  {
    ...
    else
    {
      ...
      {
#ifdef WM_DEBUG_LEVEL
        DEBUG_WM(F("No Hay WiFi Guardada, Saltando..."));
#endif
      }
    }
...
bool WiFiManager::wifiConnectNew(String ssid, String pass, bool connect)
{
  ...
  DEBUG_WM(F("Conectando Al Nuevo Punto De Acceso:"), ssid);
...
#ifdef WM_DEBUG_LEVEL
  DEBUG_WM(F("Conectando Al Punto De Acceso Guardado:"), WiFi_SSID(true));
...
String WiFiManager::WiFiManager::getScanItemOut()
{
  ...
  {
#ifdef WM_DEBUG_LEVEL
    DEBUG_WM(F("No He Encontrado Redes..."));
#endif
  ...
  {
#ifdef WM_DEBUG_LEVEL
    DEBUG_WM(n, F("Redes Encontradas..."));
#endif
    // sort networks
    int indices[n];
    for (int i = 0; i < n; i++)
...
//@todo wrap in build flag to remove all info code for memory saving
  ...
  page += F("<h3>M8AX - Acerca De - M8AX</h3><hr><dl>");
...
{
#ifdef WM_DEBUG_LEVEL
  DEBUG_WM(WM_DEBUG_NOTIFY, F("<- HTTP Borrado"));
#endif
  handleRequest();
...
#ifdef WM_DEBUG_LEVEL
  DEBUG_WM(F("Borrando Cofiguración Wifi"));
#endif
